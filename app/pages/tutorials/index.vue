<script setup>
  const contentQuery = await queryContent('tutorials/').sort(
    { timestamp: -1, }
).find()
</script>
...
<template>
  ...
  <div style="max-width:1400px; margin-left:auto; margin-right:auto; margin-top:30px">
    ...
    <template #header>
      ...
      <img class="rounded-t-lg" :src="post.image" alt="" style="height:253px;width:450px" />
      </NuxtLink>
    </template>

            <NuxtLink :to="post._path">
    <span class="text-xl font-bold" style="font-weight:500 font-size:large">{{post.title}}</span>
      </NuxtLink>
    <br>
    <br>
