...
Genie Framework provides a plugin system that allows you to extend the functionality of your apps. There exist plugins for authentication, authorisation, caching, session management, and docker image creation. See the [Plugins](/docs/api/plugins) section in the API docs for more details about each of them.

Plugins are special Julia packages which extend Genie apps with powerful functionality by providing specific integration points. A Genie plugin is made of two parts:
