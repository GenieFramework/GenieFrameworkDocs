...
[Docker](docker.com) makes container creation straightforward via its `Dockerfile` blueprint. Moreover, advanced workflows are possible such as including multiple containers running services such as databases or other backend services.
...
There exist dozens of deployment services for Docker, of which [fly.io](fly.io) is one of the easiest to use. To deploy a Genie app on this service, install the [flyctl cli](https://fly.io/docs/flyctl/) and perform these steps:
...
For instance, to check if any process is running on port 80, use the following command:
...
```
If you need to terminate a process running on port 80, you can use the following one-liner:
